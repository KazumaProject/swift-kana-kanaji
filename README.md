...
`--source` ディレクトリには Mozc の `dictionary*.txt` 群と、各補助辞書 TSV（`emoji_data.tsv` など）をまとめて置いてください。

`--no-skip-missing` フラグを付けると、TSV が存在しない補助辞書でエラーを出します。
...
let sourceDir  = URL(fileURLWithPath: "./bin_dir")
